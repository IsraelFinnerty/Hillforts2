package com.wit.hillforts.models.firebase
...
class HillfortFireStore(val context: Context) : HillfortStore, AnkoLogger {
    ...
    override fun update(hillfort: HillfortModel) {
        ...
        if ((hillfort.image1.length) > 0 || (hillfort.image2.length) > 0  || (hillfort.image3.length) > 0 || (hillfort.image4.length) > 0) {
            updateImage(hillfort)
        }
    }
